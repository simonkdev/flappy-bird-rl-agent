...
```sh
cmake -B build_check -S .
cmake --build build_check
```

The stale checked-in `build/` directory may point at a different local checkout. A fresh build directory such as `build_check/` avoids that cache issue.

## Python Demo

```sh
python3 python/demo_flappy_env.py --exe build_check/flappy_env_server
```

Run a specific number of scripted episodes:

```sh
python3 python/demo_flappy_env.py --exe build_check/flappy_env_server --episodes 10
```

Run scripted episodes until interrupted:

```sh
python3 python/demo_flappy_env.py --exe build_check/flappy_env_server --infinite
```

Pace the demo to simulated real time, useful when showing windows:

```sh
python3 python/demo_flappy_env.py --exe build_check/flappy_env_server --show-game-window --debug-window --real-time --infinite
```

Optional visual debugging:

```sh
python3 python/demo_flappy_env.py --exe build_check/flappy_env_server --debug-window --show-game-window
```
...
with FlappyEnv("build_check/flappy_env_server", seed=123) as env:
    obs = env.reset()
